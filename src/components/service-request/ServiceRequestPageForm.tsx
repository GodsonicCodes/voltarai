...
export default function ServiceRequestPageForm() {
    ...
    const handleSubmit = async () => {
        try {
            ...
            formDataToSubmit.append('company_organization_name', formData.companyName);
            formDataToSubmit.append('phone_number', `${formData.countryCode}${formData.phoneNumber}`);
            ...
            formDataToSubmit.append('preferred_communication', formData.preferredCommunication);
            formDataToSubmit.append('how_heard_about_us', formData.howDidYouHearAboutUs);
            ...
            if (result.success) {
                ...
                setErrors({ form: result.message || 'Failed to submit request. Please try again.' });
            }
        } catch (error) {
            console.error('Submission failed:', error);
            setErrors({ form: 'Failed to submit request. Please check your connection and try again.' });
        }
    };
    ...
    const renderStep = () => {
        switch (step) {
            ...
            case 3:
                return <Step3 data={formData} updateData={updateFormData} onSubmit={handleSubmit} onBack={handleBack} errors={errors} />;
            case 4:
        ...
        <div className="w-full max-w-2xl mx-auto bg-white min-h-screen sm:min-h-0 sm:h-auto sm:rounded-3xl shadow-xl overflow-hidden flex flex-col">
            {step < 4 && (
                <div className="p-6 pb-2">
                    <h1 className="text-[32px] font-bold text-gray-900 mb-8 font-sans tracking-tight">Request A Service</h1>
