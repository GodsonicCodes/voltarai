...
export default function Step1({ data, updateData, onNext, errors: parentErrors }: Step1Props) {
    ...
    const validate = () => {
        ...
        if (!data.companyName) newErrors.companyName = 'Company/Organization Name is required';
        if (!data.countryCode) newErrors.countryCode = 'Country code is required';
        if (!data.phoneNumber) newErrors.phoneNumber = 'Phone number is required';
        else if (data.phoneNumber.length < 8) newErrors.phoneNumber = 'Phone number must be at least 8 digits';
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col gap-1.5 w-full">
                ...
                <div className="flex gap-3">
                    ...
                    <div className="flex-1">
                        <input
                            ...
                            onChange={handleChange}
                            className={`flex h-12 w-full rounded-lg border bg-white px-4 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-hidden focus:ring-1 transition-colors ${getError('phoneNumber')
                ...
                {getError('phoneNumber') && <span className="text-xs text-red-500">{getError('phoneNumber')}</span>}
            </div>
            ...
            <Input
                ...
                value={data.websiteLinkedin || ''}
                onChange={handleChange}
                error={getError('websiteLinkedin')}
            />
