  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch(
        "https://volta-ai-backend.vercel.app/api/careers/",
        {
          method: "POST",
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error(
            "Failed to read error response body from careers API",
            error,
          );
        }
      ...
      if (result.success) {
        ...
        if (result.errors) {
          ...
          Object.entries(result.errors).forEach(([key, msgs]) => {
            ...
            mappedErrors[frontendKey] = Array.isArray(msgs)
              ? msgs[0]
              : (msgs as string);
          });
          ...
          const hasStep1Error = Object.keys(mappedErrors).some((k) =>
            step1Fields.includes(k),
          );
          const hasStep2Error = Object.keys(mappedErrors).some((k) =>
            step2Fields.includes(k),
          );
      ...
      setErrors({
        form: "Failed to submit application. Please check your connection and try again.",
      });
    }
  };
