...
export class VoiceAgentWebSocketService {
  ...
  async startSession(userId?: string, sessionId?: string): Promise<void> {
    try {
      // Check if the WebSocket server is reachable first
      ...
      await this.connectWebSocketWithRetry(userId, 3, sessionId);
  ...
  private async checkServerHealth(): Promise<boolean> {
    try {
      // Check if we can reach the base API endpoint first
      const response = await fetch('https://voltarai-vagent-2.onrender.com/api/health');
      if (!response.ok) {
      ...
      // Don't throw the error, just warn - we'll still try to connect
      return true; // Allow connection attempt even if health check fails
    }
  }
  
  private async connectWebSocketWithRetry(userId?: string, maxRetries: number = 3, sessionId?: string): Promise<void> {
    let attempts = 0;
    
    while (attempts < maxRetries) {
      try {
        console.log(`WebSocket connection attempt ${attempts + 1}/${maxRetries}`);
        await this.connectWebSocket(userId, sessionId);
        return; // Success, exit the loop
      } catch (error) {
        attempts++;
        console.error(`WebSocket connection failed (attempt ${attempts}/${maxRetries}):`, error);
   ...
   */
  private async connectWebSocket(userId?: string, sessionId?: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.ws.onclose = (event) => {
        ...
        // Attempt to reconnect for specific error codes (1005, 1011)
        // Avoid reconnecting on policy violations (1008) or normal closures (1000)
        ...
        // Don't reject the promise on close, as it's a normal part of the lifecycle
        // Only reject if the connection attempt failed initially
      };
    });
  }
  ...
  private async startAudioProcessing(): Promise<void> {
    ...
    this.audioProcessor.onaudioprocess = (e) => {
      ...
      // Send to server via WebSocket only if connection is open
  ...
  private async playAudio(audioData: Uint8Array): Promise<void> {
    ...
    try {
      // Create an audio buffer from the received audio data
      // Note: This is a simplified example - you may need to adjust based on actual format
      ...
      for (let i = 0; i < audioData.length; i++) {
        channelData[i] = audioData[i] / 128.0 - 1.0; // Normalize from 0-255 to -1 to 1
      }
